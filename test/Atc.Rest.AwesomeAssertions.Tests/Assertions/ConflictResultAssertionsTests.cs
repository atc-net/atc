...
public class ConflictResultAssertionsTests : ContentResultAssertionsBaseFixture
{
    ...
    public void Ctor_Sets_Subject_On_Subject_Property()
    {
        ...
        // Act
        var sut = new ConflictResultAssertions(expected, AssertionChain.GetOrCreate());
    ...
    [Theory]
    [InlineData("BAR", @"Expected content of conflict result to be string, but found System.Text.Json.JsonElement.")]
        ...
        string expectedMessage)
    {
        ...
        var sut = new ConflictResultAssertions(target, AssertionChain.GetOrCreate());
    ...
    public void WithContent_Throws_When_Content_Is_Not_Equivalent_To_Expected_WithBecauseMessage()
    {
        ...
        sut
            ...
            .Should()
            .Throw<XunitException>()
            .WithMessage(@"Expected content of conflict result to be the same string Because of something, but they differ at index 0:" + "*");
    }
